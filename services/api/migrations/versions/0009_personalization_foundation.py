...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("learning_enabled", sa.Boolean(), server_default=sa.true(), nullable=False),
        sa.Column("autonomy_level", sa.Integer(), server_default="0", nullable=False),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.CheckConstraint("autonomy_level BETWEEN 0 AND 3", name="ck_user_model_autonomy_level"),
    ...
    op.create_table(
        ...
        sa.Column("kind", sa.String(length=64), server_default="research", nullable=False),
    ...
    op.create_index("ix_goals_principal_status", "goals", ["principal_id", "status", "updated_at"])
    ...
    op.create_table(
        ...
        sa.CheckConstraint("evidence_count >= 0", name="ck_preference_claim_evidence_count"),
        sa.CheckConstraint(
            "decay_half_life_days IS NULL OR decay_half_life_days > 0",
            name="ck_preference_claim_decay",
        ),
        sa.ForeignKeyConstraint(["principal_id"], ["principals.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["user_model_id"], ["user_models.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
    ...
    op.create_index("ix_plans_principal_status", "plans", ["principal_id", "status", "updated_at"])
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["principal_id"], ["principals.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("plan_id", "step_id", name="uq_action_decision_plan_step"),
    )
    op.create_index("ix_action_decisions_plan", "action_decisions", ["plan_id", "evaluated_at"])
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("preference_claim_id", "interaction_event_id"),
    )


def downgrade() -> None:
    op.drop_table("preference_claim_evidence")
    op.drop_index("ix_outcome_events_principal_time", table_name="outcome_events")
    op.drop_table("outcome_events")
    op.drop_index("ix_action_decisions_plan", table_name="action_decisions")
    op.drop_table("action_decisions")
    op.drop_index("ix_plans_principal_status", table_name="plans")
    op.drop_table("plans")
    op.drop_index("uq_preference_claim_active_scope", table_name="preference_claims")
    op.drop_index("ix_preference_claims_principal_status", table_name="preference_claims")
    op.drop_table("preference_claims")
    op.drop_index("ix_goals_principal_status", table_name="goals")
    op.drop_table("goals")
