...
async def _citation_records(
    ...
    *,
    ...
    for work_id, display_name in author_rows:
        authors[work_id].append(_single_line(display_name))
    ...
    for entry, work, edition in rows:
        ...
        records.append(
            CitationRecord(
                ...
                title=_single_line(
                    edition.title if edition is not None else work.canonical_title
                ),
                authors=tuple(authors.get(work.id, ())),
                publication_year=edition.publication_year if edition is not None else None,
                publisher=(
                    _optional_single_line(edition.publisher) if edition is not None else None
                ),
                language=(
                    _optional_single_line(edition.language) if edition is not None else None
                ),
                edition_statement=(
                    _optional_single_line(edition.edition_statement)
...
async def _source_urls(
    ...
    *,
    ...
    for entity_type, entity_id, canonical_url, _provider in rows:
        normalized = _optional_single_line(canonical_url)
        if normalized is not None:
            result.setdefault((entity_type, entity_id), normalized)
    return result
...
def _preferred_isbn(values: list[tuple[str, str]] | tuple[tuple[str, str], ...]) -> str | None:
    ...
    candidates = [
        (priorities[scheme.casefold()], _single_line(value))
...
def _ris(records: list[CitationRecord]) -> str:
    ...
    for record in records:
        ...
        if record.source_url:
            lines.append(f"UR  - {record.source_url}")
        lines.append("ER  -")
        blocks.append("\r\n".join(lines))
    return "\r\n\r\n".join(blocks) + ("\r\n" if blocks else "")
