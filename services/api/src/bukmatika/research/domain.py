...
class ResearchEvidenceBundleRequest(BaseModel):
    ...
    @field_validator("selected_highlight_ids")
    @classmethod
...
class ResearchEvidenceItem(BaseModel):
    ...
    @model_validator(mode="after")
    def require_highlight_provenance(self) -> "ResearchEvidenceItem":
        if self.source_kind is ResearchEvidenceSourceKind.SELECTED_HIGHLIGHT:
            if self.source_highlight_id is None:
                raise ValueError("Selected highlight evidence requires its source highlight ID")
        elif self.source_highlight_id is not None:
            raise ValueError("Only selected highlight evidence may carry a source highlight ID")
        return self
