...
async def _user_model(session: AsyncSession, principal_id):  # type: ignore[no-untyped-def]
    service = PersonalizationControlService(session_scope_factory=_scope(session))
    await service.snapshot(principal_id=principal_id)
...
async def test_control_center_is_principal_scoped_and_redacts_raw_context(
    ...
    assert activity.approval_status is None
    assert activity.execution_completed is False
...
async def test_activity_ledger_shows_approved_and_completed_preference_action(
    session: AsyncSession,
) -> None:
    principal = await _principal(session, "approval-ledger")
    await _user_model(session, principal.id)
    step = PlanStep(
        step_id="remember-citation",
    ...
    plan, decisions = await PlanRepository(session).create(
        ...
        user_request="Remember Chicago citations if I approve it.",
    ...
    assert item.approval_decided_at is not None
    assert item.execution_completed is True
