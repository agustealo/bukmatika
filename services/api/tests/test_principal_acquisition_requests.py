...
async def test_failed_transfer_can_be_explicitly_requested_and_approved_again(
    session: AsyncSession,
    tmp_path,
) -> None:
    principal = await _principal(session, "failed-retry")
    asset = await _seed_open_asset(session, "707")
    service, _ = await _service(session, tmp_path)
    first = await service.request_asset(principal_id=principal.id, asset_id=asset.id)
    approved = await service.approve(principal_id=principal.id, request_id=first.request_id)
    assert approved.acquisition_id is not None

    acquisition = await _global_acquisition(session, asset.id)
    assert acquisition is not None
    acquisition.status = AcquisitionStatus.FAILED.value
    acquisition.error_code = "TEST_FAILURE"
    acquisition.completed_at = datetime.now(UTC)
    job = await JobRepository(session).get_by_dedupe_key(f"acquisition:{acquisition.id}")
    assert job is not None
    job.status = JobStatus.FAILED.value
    job.completed_at = acquisition.completed_at
    await session.flush()
    ...
    assert reapproved.acquisition_status is AcquisitionStatus.FAILED
    revived_job = await JobRepository(session).get_by_dedupe_key(f"acquisition:{acquisition.id}")
    assert revived_job is not None
    assert revived_job.status == JobStatus.QUEUED.value
    assert (
        await _event_count(
            session,
            principal_id=principal.id,
...
async def test_quarantined_transfer_is_not_reactivated_by_repeat_request(
    session: AsyncSession,
    tmp_path,
) -> None:
    principal = await _principal(session, "quarantine")
    asset = await _seed_open_asset(session, "708")
    service, _ = await _service(session, tmp_path)
    first = await service.request_asset(principal_id=principal.id, asset_id=asset.id)
    approved = await service.approve(principal_id=principal.id, request_id=first.request_id)
    assert approved.acquisition_id is not None
    ...
    acquisition = await _global_acquisition(session, asset.id)
    assert acquisition is not None
    acquisition.status = AcquisitionStatus.QUARANTINED.value
    acquisition.error_code = "TEST_QUARANTINE"
    acquisition.completed_at = datetime.now(UTC)
    await session.flush()

    repeated = await service.request_asset(principal_id=principal.id, asset_id=asset.id)
    assert repeated.status is AcquisitionRequestStatus.QUARANTINED
    assert repeated.request_id == first.request_id
    assert repeated.acquisition_id == approved.acquisition_id
    assert (
        await _event_count(
            session,
            principal_id=principal.id,
