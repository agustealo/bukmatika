...
def _evaluation(
    principal_id: UUID,
    cases: list[ResearchRecallCaseResult],
) -> ResearchRecallEvaluation:
...
def _provider() -> ModelProviderIdentity:
...
def test_comparison_rejects_different_principal_measurements() -> None:
    cases = [_case("paine-natural-paraphrase", recall=1.0, rank=1)]

    with pytest.raises(ResearchRecallSuiteInvalid):
        compare_retrieval_evaluations(
            provider=_provider(),
            lexical=_evaluation(uuid4(), cases),
            semantic=_evaluation(uuid4(), cases),
        )
